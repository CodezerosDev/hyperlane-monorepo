...
export async function getWarpConfig(
  ...
  if (envConfig.environment != 'test') {
    ...
    // let core = envConfig.core.ownerConfig;
    let core = HyperlaneCore.fromAddressesMap(
      getAddresses(envConfig.environment, Modules.CORE),
      multiProvider,
    );
    let routerConfig = core.getRouterConfig(envConfig.owners);
    const test1: TokenConfig & RouterConfig = {
      ...routerConfig.test1,
      type: TokenType.synthetic,
      symbol: 'FUSDC',
      totalSupply: 0,
